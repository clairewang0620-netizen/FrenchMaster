...
const ExamView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  if (finished) {
    return (
      <div className="max-w-xl mx-auto text-center py-16 bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_2px_6px_rgba(0,0,0,0.1)] animate-in zoom-in duration-500">
        <h2 className="text-6xl mb-6">🏆</h2>
        <h3 className="text-3xl font-extrabold text-slate-900 mb-2">挑战完成！</h3>
        <p className="text-lg text-slate-500 mb-8">
          正确率: <span className="text-blue-600 font-bold text-4xl">{Math.round((score / questions.length) * 100)}%</span>
          <br />
          <span className="text-[15px] font-medium text-slate-400">({score} / {questions.length} 题)</span>
        </p>
        <div className="flex flex-col sm:flex-row gap-4 px-8">
          <button onClick={handleReturnToLevels} className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold text-[15px] hover:bg-slate-200 transition-all">重新选择等级</button>
          <button onClick={onBack} className="flex-1 py-4 bg-[#2196F3] text-white rounded-2xl font-bold text-[15px] hover:bg-[#1e88e5] shadow-lg shadow-blue-100 transition-all">返回首页</button>
        </div>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 gap-4">
          {(['A1', 'A2', 'B1', 'B2', 'C1'] as ProficiencyLevel[]).map((lvl) => (
            <button
              ...
              className="p-6 bg-white border-2 border-slate-100 rounded-2xl flex items-center justify-between hover:border-[#2196F3] hover:scale-[1.01] transition-all group shadow-sm"
            >
              <div className="text-left">
                <span className="text-2xl font-black text-slate-300 group-hover:text-blue-500/20 transition-colors">{lvl}</span>
                <h4 className="text-lg font-bold text-slate-800">Niveau {lvl}</h4>
                <p className="text-xs text-slate-400">词汇 · 语法 · 听力</p>
              </div>
              <div className="flex items-center gap-2 text-[#2196F3] font-bold opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                开始挑战
  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center animate-in fade-in duration-500 bg-[#fff8e1] -mx-4 px-4 py-10 rounded-[3rem]">
      <div className="w-full max-w-2xl">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
              <button 
                onClick={handleReturnToLevels}
                className="px-3 py-1.5 bg-white border border-slate-200 rounded-xl text-slate-500 hover:text-blue-600 hover:border-blue-100 hover:bg-blue-50 transition-all text-xs font-bold flex items-center gap-1 shadow-sm"
              >
              ...
              <span className="bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-[10px] font-black ring-1 ring-blue-100 uppercase tracking-wider">{level}</span>
          </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-[0_2px_6px_rgba(0,0,0,0.1)] border border-slate-100 relative">
          <div className="flex justify-between items-start mb-8 gap-4">
            ...
            <button 
              ...
              className={`w-14 h-14 flex-shrink-0 flex items-center justify-center rounded-2xl transition-all shadow-lg active:scale-95 ${current.type === 'listening' ? 'bg-[#2196F3] text-white shadow-blue-100 hover:bg-[#1e88e5]' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              title="播放音频"
            >
            ...
            {current.type === 'fill_in_blank' ? (
              <div className="relative">
                <input
                  ...
                  className={`w-full p-5 rounded-2xl border-2 text-xl font-bold transition-all outline-none text-center ${
                    !isAnswered ? 'border-slate-100 bg-slate-50 focus:border-[#2196F3]' :
              ...
              <div className="grid grid-cols-1 gap-3">
                {current.options?.map((opt, i) => {
                  let stateClass = "border-slate-100 bg-white hover:border-blue-200 shadow-sm";
                  if (isAnswered) {
                    ...
                    stateClass = "border-[#2196F3] bg-blue-50 text-[#2196F3] ring-2 ring-blue-100 shadow-md font-bold";
                  }
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black uppercase transition-colors ${selectedAnswer === opt ? 'bg-[#2196F3] text-white' : 'bg-slate-100 text-slate-400 group-hover/opt:bg-blue-100 group-hover/opt:text-blue-500'}`}>
          ...
          {isAnswered ? (
            <div className="animate-in slide-in-from-top-4 duration-500 space-y-6">
              ...
              <button 
                ...
                className="w-full py-5 bg-[#2196F3] text-white rounded-2xl font-black text-[15px] shadow-xl hover:bg-[#1e88e5] transition-all transform active:scale-95"
              >
            ...
            <div className="flex gap-4">
              <button 
                ...
                className="flex-1 py-4 bg-slate-100 text-slate-400 rounded-2xl font-bold text-[15px] hover:bg-slate-200 transition-all"
              >
                跳过
              </button>
              <button 
                ...
                className="flex-[2] py-5 bg-[#2196F3] text-white rounded-2xl font-black text-[15px] shadow-xl hover:bg-[#1e88e5] disabled:opacity-30 disabled:cursor-not-allowed transition-all transform active:scale-95"
              >
                验证回答
              </button>
            </div>
