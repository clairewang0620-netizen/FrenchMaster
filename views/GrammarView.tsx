      ...
      <div className="flex gap-4 mb-8">
        {currentLevelPoints.map(p => (
          <button
            ...
            className={`flex-1 p-4 rounded-2xl border-2 transition-all font-bold text-center ${
      ...
      {selectedPoint && (
        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
          <section className="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100">
            <div className="inline-block px-3 py-1 bg-purple-50 text-purple-600 text-xs font-bold rounded-full mb-4">
               Grammaire {level}
            </div>
            <h2 className="text-3xl font-black text-slate-900 mb-4">{selectedPoint.title}</h2>
            
            <div className="space-y-4">
              <p className="text-slate-400 text-sm font-medium italic">{selectedPoint.explanation}</p>
              <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 text-slate-700 leading-relaxed font-medium">
                {selectedPoint.explanationZh}
              </div>
            </div>
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {selectedPoint.examples.map((ex, i) => (
                <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-start gap-4 group hover:border-purple-200 transition-all">
                  <div className="flex-1">
                    <p className="text-xl font-bold text-slate-900 mb-1 group-hover:text-purple-600 transition-colors">{ex.sentence}</p>
                    <p className="text-xs font-mono text-slate-400 mb-2">{ex.ipa}</p>
                    <p className="text-sm text-slate-500 font-medium">{ex.translation}</p>
                  </div>
                  <button 
                    onClick={() => speak(ex.sentence)}
                    className="p-4 bg-green-500 text-white rounded-2xl hover:bg-green-600 transition-all shadow-lg shadow-green-100 active:scale-95"
                  >
